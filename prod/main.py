...
def receber_material(conn, cursor):
    try:
        ...
        if tipo_material == 'F':
            ...
            tipo_fio_id, metragem_extraida = extrair_dados_codigo_metragem(codigo_barra) 

            #print(f"\n{AMARELO}[INFO]{RESET} Código de barras contém ID do fio: {tipo_fio_id} e metragem: {metragem_extraida}")

            if tipo_fio_id is None or metragem_extraida is None:
                print(f"\n{VERMELHO}[ERRO]{RESET} Código de barras inválido!")
                return

            resposta = input(f"\n{AMARELO}[CONFIRMAR]{RESET} A metragem é de {metragem_extraida.lstrip('0')}. Confirma? (S/N): ").strip().upper()
            if resposta == 'N':
                print(f"\n{VERMELHO}[ERRO]{RESET} Metragem não confirmada!")
                return

            codigo_convertido = tipo_fio_id + metragem_extraida
            cursor.execute("INSERT INTO Fio (codigo_barra, tipo_fio_id, metragem, em_estoque, fornecedor_id) VALUES (%s, %s, %s, TRUE, %s)", 
                           (codigo_convertido, tipo_fio_id, metragem_extraida.lstrip('0'), fornecedor_id))

            conn.commit()

        elif tipo_material == 'C':
              codigo_barra = input(f"\n{AMARELO}[INPUT]{RESET} Digite o código de barras do corante: ").strip()
              tipo_corante_id, metragem_extraida = extrair_dados_codigo_corante(codigo_barra) 
              codigo_convertido = tipo_corante_id + metragem_extraida

              print(f"\n{AMARELO}[INFO]{RESET} Código de barras contém ID do fio: {tipo_corante_id} e metragem: {metragem_extraida}")
...
def extrair_dados_codigo_corante(codigo_barra):
    try:
...
def extrair_dados_codigo_metragem(word):
    try:
        codigo_barra = [0] * 12
