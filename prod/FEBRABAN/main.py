...
def get_connection():
    try:
        ...
        cursor_teporario = conexao_temporaria.cursor()

        cursor_teporario.execute("SELECT 1 FROM pg_database WHERE datname = 'supermercado'")
        banco_existe = cursor_teporario.fetchone()
        if not banco_existe:
            cursor_teporario.execute("CREATE DATABASE supermercado")
            print("[SUCESSO] Banco de dados 'supermercado' criado.")
        else:
            print("[INFO] Banco de dados 'supermercado' já existe.")

        cursor_teporario.close()
        ...
        print("[SUCESSO] Conectado ao banco 'supermercado'.")
        ...
        with conexao.cursor() as cur:
            ...
            with open(caminho_schema, "r", encoding="utf-8") as arquivo:
                ...
                cur.execute(sql)
            conexao.commit()
            print("[SUCESSO] Script schema.sql executado.")
...
def parse_codigo_febraban(codigo):
    ...
    if not is_febraban(digitos):
        print("[ERRO] Código inválido. Deve conter 44 dígitos e iniciar com 86.")
        return None
    try:
        ...
        print(f"[ERRO] Falha ao interpretar código FEBRABAN: {e}")
        return None
...
def parse_codigo_lote_customizado(codigo):
    ...
    try:
        ...
        print(f"[ERRO] Erro ao interpretar código customizado de lote: {e}")
        return None
...
def adicionar_estoque(conexao):
    ...
    if dados["validade"] < datetime.date.today():
        print("[ERRO] Produto vencido. Entrada não permitida.")
        return

    try:
        with conexao.cursor() as cur:
            cur.execute("SELECT id FROM segments WHERE id = %s", (dados["id_segmento"],))
            if not cur.fetchone():
                cur.execute("INSERT INTO segments (id, name) VALUES (%s, %s)", (dados["id_segmento"], f"Segmento {dados['id_segmento']}"))

            cur.execute("SELECT id FROM empresas WHERE cnpj LIKE %s", (dados["cnpj8"] + '%',))
            row = cur.fetchone()
            if row:
                empresa_id = row[0]
            else:
                cur.execute("INSERT INTO empresas (name, id_segmento, cnpj) VALUES (%s, %s, %s) RETURNING id",
            ...
            if preco_existente and preco_existente[0] != dados["valor"]:
                print("[ERRO] Valor do lote difere do preço cadastrado. Entrada não permitida.")
                return

            if not preco_existente:
                cur.execute("INSERT INTO produtos (id, code, name, preco, company_id) VALUES (%s, %s, %s, %s, %s)",
            ...
            if cur.fetchone():
                print("[ERRO] Este lote já foi registrado anteriormente.")
                return
            ...
            conexao.commit()
            print(f"[SUCESSO] {dados['quantidade']} unidades do produto {dados['id_produto']} adicionadas ao estoque.")
    except Exception as e:
        print(f"[ERRO] Erro ao processar lote: {e}")
        conexao.rollback()
...
def buscar_produto(conexao, id_produto):
    try:
        with conexao.cursor() as cur:
            cur.execute("""
                SELECT p.id, p.name, p.preco, COALESCE(s.quantidade, 0)
                FROM produtos p
            ...
            res = cur.fetchone()
            if res:
                return {"id": res[0], "name": res[1], "preco": res[2], "estoque": res[3]}
            return None
    except Exception as e:
        ...
        with conexao.cursor() as cur:
            ...
            while True:
                ...
                if is_febraban(codigo):
                    ...
                    if valor is None:
                        print("[ERRO] Código FEBRABAN inválido.")
                        continue
                ...
                if dados:
                    ...
                    if produto["estoque"] <= 0:
                        print(f"[ERRO] Produto '{produto['name']}' sem estoque.")
                        continue
                    cur.execute("""
                    ...
                    total += produto["preco"]
                    produtos.append((produto["name"], produto["preco"]))
                    print(f"[Produto] {produto['name']} | R$ {produto['preco']:.2f}")
                else:
                    print("[ERRO] Formato de código inválido.")
            conexao.commit()
    except Exception as e:
        print(f"[ERRO] Falha no checkout: {e}")
        conexao.rollback()
