...
-- Sequência para a tabela PATROCINADOR
CREATE SEQUENCE SEQ_PATROCINADOR_ID
  START WITH 1
  INCREMENT BY 2
  NOCACHE
  NOCYCLE;

-- Sequência para a tabela PAIS
...
-- Sequência para a tabela ORGANIZADOR_GERAL
CREATE SEQUENCE SEQ_ORGANIZADOR_GERAL_ID
  START WITH 1
  INCREMENT BY 4
  NOCACHE
  NOCYCLE;

-- Sequência para a tabela MODALIDADE
CREATE SEQUENCE SEQ_MODALIDADE_ID
  START WITH 1
  INCREMENT BY 5
  NOCACHE
  NOCYCLE;

-- Sequência para a tabela MIDIA
CREATE SEQUENCE SEQ_MIDIA_ID
  START WITH 1
  INCREMENT BY 6
  NOCACHE
  NOCYCLE;

-- Sequência para a tabela LOCAL
...
-- Sequência para a tabela DISPUTA
CREATE SEQUENCE SEQ_DISPUTA_ID
  START WITH 1
  INCREMENT BY 8
  NOCACHE
  NOCYCLE;
...
CREATE TABLE ATLETA(
    ...
    CONSTRAINT IDADE_CK CHECK(IDADE >= 10 AND IDADE <= 100)
);
...
CREATE TABLE PAIS(
    ...
    NOME VARCHAR2(60) NOT NULL,
    CONTINENTE CHAR(2),
    POPULACAO INTEGER DEFAULT 5000,
...
CREATE TABLE TRANSMISSAO_TV(
    ...
    CONSTRAINT TRANMISSAO_PK PRIMARY KEY(DISPUTA, MIDIA),
    CONSTRAINT TRANMISSAO_FK1 FOREIGN KEY(DISPUTA) REFERENCES DISPUTA(ID) ON DELETE CASCADE,
    CONSTRAINT TRANSMISSAO_FK1 FOREIGN KEY(MIDIA) REFERENCES MIDIA(ID) ON DELETE CASCADE
);
...
CREATE TABLE RADIO(
    ...
    CONSTRAINT RADIO_PK PRIMARY KEY(NOME),
    CONSTRAINT RADIO_UN UNIQUE(FREQUENCIA),
    CONSTRAINT RADIO_FK FOREIGN KEY(NOME) REFERENCES MIDIA(NOME) ON DELETE CASCADE,
    CONSTRAINT RADIO_CK CHECK(FREQUENCIA > 0.00)
);
